...
Git is a version control tool. Github is a website on which you can share your R package and other code with others. Evan Poworoznek has created a Github folder for [PRIME](https://github.com/niehs-prime). You could cantact him (Github username:@brjoubert) to add your github account to the project so that you could add your own project inside folder.

Good news is Rstudio is able to work with Git and Github well. However, you may need to be familiar with the workflow of the Github. 
...
After you upload your package on the Github website, it is very straightforward to add it into the `prime.master` package. You need to edit the `DESCRIPTION` file of the `prime.master` package. In the following case, I add the `prime.test` into the `prime.master` via the `Remote` section. 
...
1. Pick a version number
    * Define a sequence of number in the `Version` section of the `DESCRIPTION` file. 
    * The format is `major.minor.patch`, e.g. 1.8.2
1. Run and document `R CMD check`
    * Operation system: MAC, Windows and Linux. (working at least under two environment)
...
The further detail introduction of releasing a package could be found [here](https://r-pkgs.org/release.html)
